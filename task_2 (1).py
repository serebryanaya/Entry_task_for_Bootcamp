...
#иассив для операций act, массив для чисел/граней - digit
list_action = []
list_digit = []

#сейчас не используется. но вдруг пригодится. она тупо говорит, что за чар пришел: оператор или нет 
def inset(elem):
	if elem == '(' or elem == ')' or elem == '*'\
		 or elem == '+' or elem == '-' or elem == '>' or elem == 'd':
		 return True
	return False
...
i = 0
#цикл идет по строке формулы (str = input()) и распихивает на 2 листа: act - стек для операций (никогда не может быть чисел и граней), digit - стек для чисел/граней (+ тут могут временно быть операторы) 
while i <= len(str) - 1:
